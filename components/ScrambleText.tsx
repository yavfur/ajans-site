...
// ScrambleText is desktop-only. On mobile, renders text directly for performance.
...
export default function ScrambleText({
  ...
  useEffect(() => {
    ...
    function tick() {
      const now = Date.now();
      if (now < startAt) { frameRef.current = setTimeout(tick, 16); return; }
      ...
      setDisplayed(result);
      if (now < endAt) { frameRef.current = setTimeout(tick, 32); }
